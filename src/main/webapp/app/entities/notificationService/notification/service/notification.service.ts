...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...
      lastModifiedDate: notification.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...
      lastModifiedDate: restNotification.lastModifiedDate ? dayjs(restNotification.lastModifiedDate) : undefined,
    };
  }
