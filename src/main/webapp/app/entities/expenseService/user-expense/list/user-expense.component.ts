...
export class UserExpenseComponent implements OnInit {
  ...
  trackId = (_index: number, item: IUserExpense): string => this.userExpenseService.getUserExpenseIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(userExpense: IUserExpense): void {
    const modalRef = this.modalService.open(UserExpenseDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userExpenses = this.refineData(dataFromBody);
  }

  protected refineData(data: IUserExpense[]): IUserExpense[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IUserExpense[] | null): IUserExpense[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.userExpenseService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
